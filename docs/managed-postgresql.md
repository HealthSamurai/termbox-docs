...
Termbox can connect to any PostgreSQL-compatible database, both self-hosted and managed. Database connections are configured with the [`PG_*`](configuration.md#database-settings) environment variables. Connecting to a typical PostgreSQL instance may require the following setup:
